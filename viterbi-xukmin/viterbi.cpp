...
ViterbiCodec::ViterbiCodec(int constraint, const std::vector<int>& polynomials)
    : constraint_(constraint), polynomials_(polynomials),encodeingstate(0),paddinglength(24) {
...
//jonti
QVector<int> ViterbiCodec::Decode_Continuous(const QVector<int>& bits)
{
    int k=(polynomials_.size()*(paddinglength+constraint_));
    decode_padding.resize(k);
    for(int i=0;i<bits.size();i++)decode_padding.push_back(bits[i]+'0');
    std::string decoded=Decode(decode_padding).substr(paddinglength+1);
    QVector<int> ret;for(int i=0;i<decoded.size();i++)ret.push_back(decoded[i]-'0');
    decode_padding=decode_padding.substr(decode_padding.size()-k);
    return ret;
}
...
QVector<int> ViterbiCodec::Encode_Continuous(const QVector<int>& bits)
{
    ...
    for (int i = 0; i < bits.size(); i++)
    {
      ...
      tmp = Output(encodeingstate, input);
      for(int j=0;j<num_parity_bits();j++)
      {
...
//jonti
std::string ViterbiCodec::Decode_Continuous(const std::string& bits)
{
    int k=(polynomials_.size()*(paddinglength+constraint_));
    decode_padding.resize(k);
    decode_padding+=bits;
    std::string ret=Decode(decode_padding).substr(paddinglength+1);
    decode_padding=decode_padding.substr(decode_padding.size()-k);
    return ret;
}

//jonti
std::string ViterbiCodec::Encode_Continuous(const std::string& bits)
{
  std::string encoded;
  ...
  for (int i = 0; i < bits.size(); i++)
  {
    ...
    assert(c == '0' || c == '1');
    int input = c - '0';
    encoded += Output(encodeingstate, input);
    encodeingstate = NextState(encodeingstate, input);
  }
